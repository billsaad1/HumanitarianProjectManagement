...
namespace HumanitarianProjectManagement.Forms
{
    public partial class ProjectMonitoringForm : Form
    {
        ...
        {
            string[] columnsToHide = { "ProjectIndicatorID", "ProjectID", "Project", "VerificationMeans", "Description", "BaselineValue", "StartDate", "EndDate" };
        ...
        private async void btnEditIndicator_Click(object sender, EventArgs e)
        {
            if (dgvProjectIndicators.SelectedRows.Count > 0)
            {
                ...
                try
                {
                    indicatorToEdit = await _monitoringService.GetIndicatorByIdAsync(selectedSummary.ProjectIndicatorID);
                }
                catch (Exception ex)
                {
        ...
        private async void btnDeleteIndicator_Click(object sender, EventArgs e)
        {
            if (dgvProjectIndicators.SelectedRows.Count > 0)
            {
                ...
                if (confirmation == DialogResult.Yes)
                {
                    ...
                    try
                    {
                        success = await _monitoringService.DeleteProjectIndicatorAsync(selectedIndicator.ProjectIndicatorID);
                        if (success)
                        {
