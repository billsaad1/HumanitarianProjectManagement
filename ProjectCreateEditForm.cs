    ...
    public partial class ProjectCreateEditForm : Form
    {
        ...
        private void RenderAllOutcomes()
        {
            ...
            foreach (var outcome in _currentProject.Outcomes.ToList()) // ToList for safe iteration if collection modified by delete
            {
                ...
                PlaceholderTextBox txtOutcomeDesc = new PlaceholderTextBox // This was confirmed to be separated by user's version
                {
                ...
                txtOutcomeDesc.PlaceholderText = "Enter outcome description";
        ...
        private void BtnDeleteOutcome_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show( // User confirmed this formatting is fine
                $"Are you sure you want to delete this outcome and all its outputs, activities, and indicators?" + 
        ...
        private void BtnAddOutputToOutcome_Click(object sender, EventArgs e)
        {
            ...
            Output newOutput = new Output { OutcomeID = outcome.OutcomeID }; 
        ...
        private void RenderOutputsForOutcome(Outcome outcome, Panel parentOutputPanel) 
        {
            ...
            foreach (var outputInstance in outcome.Outputs.ToList()) 
            {
                ...
                TextBox txtOutputDesc = new TextBox
                {
                    ...
                    ScrollBars = ScrollBars.Vertical,
                    Height = 40,
                    Width = parentOutputPanel.ClientSize.Width - 15, 
        ...
        private void BtnAddIndicator_Click(object sender, EventArgs e)
        {
            ...
            ProjectIndicator newIndicator = new ProjectIndicator { ProjectID = _currentProject.ProjectID }; 
            output.ProjectIndicators.Add(newIndicator);

            Panel pnlDynamicOutput = btn.Parent.Parent as Panel; 
            Panel pnlIndicatorsContainer = pnlDynamicOutput.Controls.OfType<Panel>().FirstOrDefault(p => p.Name == null && p.Location.Y > btn.Parent.Bottom); 
        ...
        private void RenderIndicatorsForOutput(Output output, Panel parentIndicatorPanel)
        {
            ...
            foreach (var indicatorInstance in output.ProjectIndicators.ToList())
            {
                ...
                tlpIndicator.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute, 100)); 
                tlpIndicator.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 100)); 

                PlaceholderTextBox txtIndicatorName = new PlaceholderTextBox // This was confirmed to be separated by user's version
                { 
                    Text = indicatorInstance.IndicatorName, 
                    Dock = DockStyle.Fill
                };
                txtIndicatorName.PlaceholderText = "Indicator Description";
                ...
                NumericUpDown nudTargetTotal = new NumericUpDown { Value = indicatorInstance.TargetTotal, Maximum = 4000000, Width = 70, Enabled = false }; 
        ...
        private void BtnAddActivity_Click(object sender, EventArgs e)
        {
            ...
            Panel pnlDynamicOutput = btn.Parent.Parent as Panel;
            Panel pnlActivitiesContainer = pnlDynamicOutput.Controls.OfType<Panel>().LastOrDefault(p => p.Name == null && p.Location.Y > btn.Parent.Bottom); 
        ...
        private void RenderActivitiesForOutput(Output output, Panel parentActivityPanel)
        {
            ...
            foreach (var activityInstance in output.Activities.ToList())
            {
                ...
                PlaceholderTextBox txtActivityDesc = new PlaceholderTextBox
                {
                    ...
                    Dock = DockStyle.Fill // PlaceholderText removed from this initializer
                };
                txtActivityDesc.PlaceholderText = "Activity Description"; // PlaceholderText is now assigned on a separate line
        ...
        private async Task LoadComboBoxesAsync() 
        {
            ...
            if (_isEditMode && _currentProject.SectionID != null)
            {
                ...
                if (cmbSection.SelectedItem == null) cmbSection.SelectedIndex = 0; 
            }
            ...
            if (_isEditMode && _currentProject.ManagerUserID != null)
            {
                foreach (ComboboxItem item in cmbManager.Items)
                {
                 ...
                 if (cmbManager.SelectedItem == null) cmbManager.SelectedIndex = 0; 
            }
            else
            {
